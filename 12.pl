...
# Lets go for the unoptimized approach and read later :D
# Afterwards, it wasn't that bad! Hint: Prime Decomposition
my $number_of_divisors = 500;
sub g {
  my $n = shift;
  return $n * ($n + 1) / 2;
}
for (my $i = 0;; $i++) {
  my $counter = 0;
  my $tri = g($i);
  my $sqrt = ceil(sqrt($tri));
  for (1..$sqrt) {
    $counter+=2 unless ($tri % $_);
  }
  $counter-- if ($sqrt * $sqrt == $tri);
  if ($counter >= $number_of_divisors) {
    print "$tri\n";
    exit 0;
  }
}
